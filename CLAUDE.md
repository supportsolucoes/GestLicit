...
- `contratos` + `contrato_itens` — contratos formais firmados após licitação ganha. `contratos.licitacao_id` é **obrigatório** (`not null`, diferente de `atas.licitacao_id` que é opcional) — todo contrato precisa referenciar a licitação que o originou. Sem saldo/consumo (diferente de Ata): itens têm quantidade e valor unitário fixos, `valor_total` nunca é armazenado — é sempre `Σ valor_unitario × quantidade_total`, calculado em `modules/contratos.js`. `contrato_itens.produto_id` é obrigatório (mesma regra de `licitacao_itens`, sem opção de texto livre).
...
- Ao gerar uma Ata a partir de uma Licitação ganha, pré-popular os itens da ata a partir dos itens da licitação com status "Ganhou" — o mesmo mecanismo já existe para Contratos (`carregarItensLicitacao` em `modules/contratos.js`); falta replicar/adaptar para `modules/atas.js`.
